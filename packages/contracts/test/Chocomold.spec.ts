...
const defaultBaseUrl =
    "https://asia-northeast1-chocofactory-prod.cloudfunctions.net/metadata/";
const chainId = 31337;
...
describe("Chocomold", function () {
    ...
    this.beforeEach("initialization.", async function () {
        ...
        const deployedMold = await factory.predictDeployResult(
            mold.address,
            signer.address,
            MODAL_NAME,
            MODAL_SYMBOL
    ...
    it("interface check", async function () {
        expect(
            await moldContract.supportsInterface(ERC721_INTERFACE_ID)
        ).to.equal(true);
        expect(
            await moldContract.supportsInterface(ERC721_METADATA_INTERFACE_ID)
        ).to.equal(true);
        expect(
            await moldContract.supportsInterface(HAS_SECONRARY_SALE_FEES)
        ).to.equal(true);
    });
    it("get no royality when no default, no custom", async function () {
        ...
        const feeRecipientsResult = await moldContract.getFeeRecipients(
            tokenId
        );
    ...
    it("get default royality when default set, no custom", async function () {
        ...
        await moldContract["mint(address,uint256)"](signer.address, tokenId);
        await moldContract.setDefaultRoyality(
    ...
    it("set default royality fails with diffent length input", async function () {
        ...
        await moldContract["mint(address,uint256)"](signer.address, tokenId);
        await expect(
            moldContract.setDefaultRoyality(
                defaultRoyaltyAddress,
                defaultRoyalty
            )
    ...
    it("revert token url when token not exist", async function () {
        await expect(moldContract.tokenURI(tokenId)).to.revertedWith(
            "token must exist"
        );
    });
    ...
    it("bulk mint fali with different length input", async function () {
        ...
        await expect(
            moldContract["mint(address[],uint256[])"](toList, tokenIdList)
        ).to.revertedWith("input length must be same");
    });
    it("bulk mint, bulk set royality", async function () {
        ...
        ](tokenIdList, royalityAddressList, royalityList);
    });
    ...
    it("bulk mint, bulk set ipfs fail with different length input", async function () {
        ...
        await moldContract["mint(address[],uint256[])"](toList, tokenIdList);
        await expect(
            moldContract["setIpfsHash(uint256[],bytes32[])"](
                tokenIdList,
                ipfsHashList
            )
        ).to.revertedWith("input length must be same");
    ...
    it("check burn is possible and deleted royality and ipfs", async function () {
        ...
        await expect(moldContract.ownerOf(tokenId)).to.revertedWith(
